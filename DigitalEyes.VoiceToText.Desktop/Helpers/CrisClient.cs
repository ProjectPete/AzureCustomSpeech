namespace DigitalEyes.VoiceToText.Desktop.Helpers
{
    ...
    //    private async Task<Uri> PostAsAsync<TPayload>(string path, HttpContent payload)
    //    {
    //        using (var response = await this.client.PostAsync(path, payload).ConfigureAwait(false))
    //        {
    //            return await GetLocationFromPostResponseAsync(response).ConfigureAwait(false);
    //        }
    //    }

    //    private async Task<string> GetAsync<TResponse>(string path)
    //    {
